"""NAdam optimizer with reduce LR on Plateau in Jax."""
import functools
import logging
...
def nadam(
  ...
  # Create the NAdam transform
  nadam_transform = scale_by_nadam(b1, b2, eps, eps_root, debias)
  
  # Create the learning rate scaling transform
  lr_transform = scale_by_learning_rate(learning_rate)
...
def update_params(workload: spec.Workload,
  ...
  reduce_lr_on_plateau_state = new_optimizer_state_host[0]

  if hasattr(reduce_lr_on_plateau_state, 'scale'):
      scale = reduce_lr_on_plateau_state.scale
      new_learning_rate = hyperparameters['learning_rate'] * scale
      ...
      log_reduce_lr_state(global_step, scale, best_value, plateau_count, 
                        cooldown_count, reduce_lr_log_file)
  else:
      # Handle initialization case or if state structure is different
      new_learning_rate = hyperparameters['learning_rate']
      hcb.id_tap(log_learning_rate_callback, (global_step, new_learning_rate))
      log_reduce_lr_state(global_step, 1.0, float('inf'), 0, 0, reduce_lr_log_file)
